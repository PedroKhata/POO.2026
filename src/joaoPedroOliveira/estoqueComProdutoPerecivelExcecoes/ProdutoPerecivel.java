...
public class ProdutoPerecivel extends Produto {
    ...
    public double venda(int quant, Date dataAtual) throws ProdutoVencido, DadosInvalidos {
        int aux = quant;
        if (verificarQuantValidos(dataAtual) < quant) {
            throw new ProdutoVencido(quant, verificarQuantValidos(dataAtual));
        }
        if (quant <= 0) {
